        ...
        <v-card class="ld-modal">
          ...
          <v-card-actions>
            <v-row no-gutters>
              ...
              <v-col cols="12" md="6" class="pa-1">
                <v-btn class="btn-secondary" @click.prevent="closeModal"
                  >CANCELAR</v-btn
                >
              </v-col>
            </v-row>
...
import { async } from 'q'
import DocumentoFirmar from '@/components/contigo/DocumentoFirmar'
...
export default {
  ...
  computed: {
    ...
    isSelectALL() {
      if (this.isCheckValorados.length === 4) {
        this.$store.commit('contigo/UPDATE_DATOS', {
    ...
    validarToken() {
      this.$validator.validateAll().then(async (result) => {
        if (result) {
          ...
          this.$store
            .dispatch(
              'contigo/validarGenerarContractuales',
