...
<script>
import OfertaItem from '@/components/contigo/OfertaItem'
...
export default {
  ...
  methods: {
    ...
    continuar() {
      try {
        this.loading = true
        this.$store
          .dispatch('contigo/elegirOferta', JSON.stringify(this.ofertaForm))
          .then((res) => {
            if (res.codRes == '00') {
              this.$store.commit('contigo/SET_COD_EVALUACION', res)
              this.$store.commit('contigo/SET_STEP', 4)
  ...
  props: ['color'],
  computed: {
    ...mapState('contigo', ['ofertas', 'body_valida']),
  },
  mounted() {},
  data() {
    return {
