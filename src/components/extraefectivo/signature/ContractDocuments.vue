...
import DocumentoFirmar from '@/components/extraefectivo/signature/DocumentoFirmar'
...
export default {
  ...
  computed: {
    ...
    isSelectALL() {
      if (
        ...
        this.$store.commit('extraefectivo/UPDATE_DATOS', {
  ...
  methods: {
    ...
    async firmarContractuales() {
      this.loading = true
      await this.$store.dispatch('extraefectivo/actionFacialBiometryRecord', {
