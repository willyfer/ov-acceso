<template>
    ...
    <v-card class="ld-modal" max-width="500" style="margin-top: 0%">
        ...
        <v-card-text  style="padding-top: 0px !important; margin-top: -20px ">
            ...
            <v-card-text style="padding-top: 0px !important; margin-top: -10px ">
          ...
          <v-row no-gutters>
            <v-col cols="12" order-md="2" md="6" class="text-center pa-1">
              <v-btn  class="ld-btn" style="border-radius:5px; background-color: #FF7729;" 
...
export default {
  ...
  data() {
    return {
      slogan: `Debes registrar solo la cuenta bancaria del titular del DNI para que el préstamo se active. <br> 
      ...
      disabledText: true,
            ...
            rulesCI : [
              v => !!v || 'Campo obligatorio',
              v => ( v && v.length >= this.longitudMax || (v.length == 10 && this.selectEntidadBancaria.codigoBanco == 3) ) || 'Cantidad de digitos no alcanzada'
              
            ],
            rulesCCI : [
              v => !!v || 'Campo obligatorio',
              v => ( v && v.length >= this.longitudMaxInterbancaria ) || 'Cantidad de digitos no alcanzada'
            ],
            tipoCuentaBancaria: '',
            longitudMax: 0,
            entidadBancariaOrigen: 0,
         ...
         if(result){ 
           var regex_numeros = /^[0-9]+$/
            ...
            var validateCII = regex_numeros.exec(this.newCCI)      
              if(this.newCI.length < this.longitudMax || this.newCCI.length < this.longitudMaxInterbancaria){
                if(this.newCI.length == 10 && this.newCCI.length == this.longitudMaxInterbancaria && this.codigoBanco == 3){
                      const dataSend = {
                ...
                const msm = {
                  msm : "La cuenta bancaria debe contener solo números, por favor inténtelo nuevamente"
                }
                this.$emit('modalNoCoinciden', msm)                
              }else if (!validateCII) {
                const msm = {
                  msm : "La cuenta interbancaria debe contener solo números, por favor inténtelo nuevamente"
                }
                this.$emit('modalNoCoinciden', msm)  
              }else{
                  const dataSend = {
                   ...
                   //   console.log(dataSend)
                       const body = {
                          codigoContrato: this.sendResponseData.codigoContrato,
                          entidadBancariaOrigen: this.entidadBancariaOrigen,
                          codigoBancoPagar: this.entidadBancariaOrigen,
                          entidadBancariaDestino: this.codigoBanco,
                          tipoCuenta: this.selectTipoCuenta.id,//cuenta corriente o ahorros
                          numeroCuentaBancaria: this.newCI ? this.newCI : null,
                          numeroCuentaInterbancaria: this.newCCI ? this.newCCI : null
           ...
           if(this.newCI.length < this.longitudMax || this.newCCI.length < this.longitudMaxInterbancaria){            
                  const msm = {
                    msm : "Por favor compruebe que los campos esten correctamente"
                  }
                  this.$emit('modalNoCoinciden', msm)
           }
         }
            ...
            if(res){
               ...
               //   console.log(res.cuentaInterbancaria)
                   ...
                   //   console.log(" cuando se cambia de entidadBancariaOrigen" ,this.entidadBancariaOrigen)
                       //this.longitudMax = 20
                         this.tipoCuentaBancaria == '' ? this.tipoCuentaBancaria = 1 : this.tipoCuentaBancaria = this.tipoCuentaBancaria
