  ...
  <v-card class="text-center ma-1">
    ...
    <v-card-actions>
      ...
      <a
        ...
        :href="
          `${url}/api/acr/${this.item.doc}/${this.codigoExpediente}/0`
        "
        target="_blank"
        @click="changeState"
...
export default {
  name:'DocumentoFirmarConversiones',
  props: ['item', 'maxItems', 'codigoExpediente'],
  data() {
    return {
      actions: false,
      signature: false,
  ...
  methods: {
    changeState() {
      ...
      if (!this.signature) {
        ...
        this.$store.commit('conversiones/SET_DATOS', { isCheckValorados })
      }
      this.signature = true
    }
  }
