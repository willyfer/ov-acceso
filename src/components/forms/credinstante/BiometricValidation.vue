...
export default {
  ...
  methods: {
    ...
    onExtractionFinish(value) {
      const payload = {
        ...value,
        codigoProspecto: this.datos.codigoProspecto,
        documento: this.datos.documento,
      }
      this.$store.dispatch('express/actionFacialBiometriy', payload)
    },
    async onExceptionCaptured(value) {
      console.log('onExceptionCaptured: ', value.detail)
      await this.$store.dispatch('express/actionExceptionCaptured', {
        codigoProspecto: this.datos.codigoProspecto,
        documento: this.datos.documento,
        metodo: 3,
  ...
  beforeDestroy() {
    console.log('beforeDestroy en BiometricValidation')
  },
}
</script>
